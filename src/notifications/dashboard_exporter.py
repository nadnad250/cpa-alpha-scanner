            ...
            try:
                ...
                for ts in tracker_opens:
                    ...
                    existing_open_by_key[key] = {
                        ...
                        "risk_reward":    getattr(ts, "risk_reward", None) or rrT,
                        "kelly_position": round(kellyT, 3),
                        "cpa_alpha":      None,
                        # Bug #5 : on lit le ml_proba_up persisté
                        "ml_proba_up":    getattr(ts, "ml_proba_up", None),
                        # Bug #2 : on lit le secteur persisté (au lieu de "—" en dur)
                        "sector":         getattr(ts, "sector", "") or "—",
                        "universe":       ts.universe or "—",
                        "primary_reason": getattr(ts, "primary_reason", "") or "Signal CPA Alpha (actif)",
                        "secondary_reasons": [],
                        "risk_flags":     [],
